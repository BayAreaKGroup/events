...
import { donationSupportCopy, type Locale } from '@/content/siteContent'

/** Figma section-donation-support 562:7233 — viewport 1440, content 1160 (1200−40) */
export default function SectionDonationSupport({ locale }: { locale?: Locale }) {
  const copy = locale ? donationSupportCopy[locale] : null
  return (
    <section
      id="donation-support"
          ...
          >
            <StaggerItem>
              ...
              >
                ...
                >
                  {copy?.onlineTitle ?? '간편한 온라인 기부 · Online Donation'}
                </h3>
                <div
                  className="type-body"
                  data-node-id="562:7326"
                >
                  <p>{copy?.onlineBody ?? '원하시는 플랫폼을 선택하시면 바로 기부 페이지로 연결됩니다.'}</p>
                  <p>{copy?.onlineNote ?? '(Zeffy 수수료 전액 면제 플랫폼)'}</p>
                </div>
                <div
            ...
            <StaggerItem>
              ...
              >
                ...
                >
                  {copy?.checkTitle ?? '수표 기부 · Check Donation'}
                </h3>
                <div
                  className="type-body"
                  data-node-id="562:7334"
                >
                  <p>{copy?.checkBody ?? 'K-Group 앞으로 수표를 보내실 수 있습니다.'}</p>
                  <ul className="list-disc space-y-0 pl-[27px]">
                    <li>{copy?.payee ?? 'Payee (수취인): Bay Area K-Group'}</li>
                    <li>
                      {copy?.address ?? 'Mailing Address (주소): P.O Box 60424, 209 East Java Dr., Sunnyvale, CA 94086'}
                    </li>
                  </ul>
                </div>
              </div>
            ...
            <StaggerItem>
              ...
              >
                ...
                >
                  {copy?.matchingTitle ?? '기부금 매칭 · Corporate Matching'}
                </h3>
                ...
                >
                  ...
                  >
                    {copy?.matchingHighlight ?? '회사 베네핏으로 후원 효과 200% 높이기'}
                  </h4>
                  <p
                  className="type-body"
            ...
            <StaggerItem>
              ...
              >
                ...
                >
                  {copy?.volunteerTitle ?? '자원봉사 지원금 · Volunteer Grant'}
                </h3>
                <p
                  className="type-body"
        ...
        <Reveal variants={fadeUpScale} delay={0.08}>
          ...
          >
            ...
            <div className="relative z-10 flex w-full max-w-[578px] flex-col items-start gap-8 px-5 py-10 text-left sm:gap-12 sm:px-8 sm:py-12 md:gap-12 md:px-[97px] md:py-[49px]">
              ...
              >
                <p>
                  {copy?.ctaBody[0] ?? 'K-Group을 통해 좋은 인연을 만나고 함께 성장한 경험이 있다면, 이제 그 기회를 다음 사람에게 이어주세요.'}
                </p>
                <p className="mt-4">
                  {copy?.ctaBody[1] ?? '여러분의 작은 후원이 실리콘밸리에서 더 많은 연결을 만들고, 더 큰 성장의 계기가 됩니다.'}
                </p>
              </div>
              ...
              >
                {copy?.ctaButton ?? 'Support K-Group'}
              </a>
            </div>
          </div>
        </Reveal>
